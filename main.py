...
def extract_tags(text, max_retries=8, wait_seconds=10):
    prompt = f"""النص التالي هو خبر تقني:
{text}
استخرج فقط أسماء الأدوات أو الخدمات أو المنصات أو البرامج أو الأنظمة أو الأجهزة أو الشركات أو المواقع أو نماذج الذكاء الاصطناعي التي ترتبط مباشرة بموضوع الخبر أو التقنية أو الحدث الرئيسي، واكتبها كما وردت بالإنجليزية فقط، بدون أي تعريب أو ترجمة أو شرح، وفقط مفصولة بمسافة واحدة، ولا تكرر أي اسم. تجاهل الأسماء التي وردت بشكل عابر أو غير جوهري."""
    tags_text = gemini_ask(prompt, max_retries, wait_seconds)
    if tags_text:
        cleaned = re.sub(r'[^\w\s]', '', tags_text)  # يسمح فقط بالحروف والأرقام والمسافة
